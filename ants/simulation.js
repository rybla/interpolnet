...
class Ant {
    ...
    steer(grid, pheromoneToFollow, target) {
        ...
        if (this.isTargetAt(grid, aheadPos.x, aheadPos.y, target)) aheadPheromone += 10;
        if (this.isTargetAt(grid, leftPos.x, leftPos.y, target)) leftPheromone += 10;
        if (this.isTargetAt(grid, rightPos.x, rightPos.y, target)) rightPheromone += 10;
        ...
        if (aheadPheromone > leftPheromone && aheadPheromone > rightPheromone) {
            ...
            this.direction += (Math.random() - 0.5) * 0.8; // Random turn
        }
    }
